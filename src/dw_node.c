...
int reply(req_info_t *req, message_t *m, command_t *cmd, conn_worker_info_t* infos) {
    ...
    case REPLY_MODE_SENDFILE:
        ...
        req->sendfile_fd = storage_worker_info[dev_id].storage_info.storage_fd;//conn_worker_infos[conn_id].storage_fd; // note! fd must be copied here since req will be freed after this function returns,but the sendfile operation may not be completed yet
...
static error_t argp_node_parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    case STORAGE_OPT_ARG:
        ...
        storage_worker_info_t *sw_info = &storage_worker_info[storage_devices];
        *sw_info = (storage_worker_info_t) {
            .timerfd = -1,
            .periodic_sync_msec = -1,
            .use_wait_spin = -1,
            .core_id = -1,
            .count = 1, // not settable globally
            .storage_info = {
                .storage_fd = -1,
                .storage_path = "",
                .use_odirect = -1,
        ...
        do { // iterate through comma-separated fields of --storage argument
            ...
            if (strncmp(tok, "file=", 5) == 0) {
                ...
                if (tok[5] == '/') {
                    ...
                    snprintf(sw_info->storage_info.storage_path, MAX_STORAGE_PATH_STR, "%s/%s", cwd, tok + 5);
                }
            }
                ...
                dw_log("Invalid --storage option argument (should be file=, size=, count=, sync=, odirect, or wait-spin): %s\n", tok);
                exit(EXIT_FAILURE);
            }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < storage_devices; i++) {
        ...
        char path_template[MAX_STORAGE_PATH_STR];

        if (s_info->storage_path[0] == '\0') {
            char *home_path = getenv("HOME");
            check(home_path != NULL && strlen(home_path) + 20 < sizeof(*s_info));
            snprintf(path_template, MAX_STORAGE_PATH_STR, "%s/.dw_store_%%d", home_path);
            snprintf(s_info->storage_path, MAX_STORAGE_PATH_STR, path_template, i);
        } else {
            // look for %d in the path to replace with storage index, otherwise append it
            if (strstr(s_info->storage_path, "%d") == NULL) {
                snprintf(path_template, MAX_STORAGE_PATH_STR, "%s_%%d", s_info->storage_path);
            } else {
                strncpy(path_template, s_info->storage_path, MAX_STORAGE_PATH_STR);
            }
            snprintf(s_info->storage_path, MAX_STORAGE_PATH_STR, path_template, i);
        }
        ...
        for (int j = 1; j < sw_info->count; j++) {
            ...
            storage_worker_info_t *sw_info2 = &storage_worker_info[i+j];
            ...
            snprintf(s_info2->storage_path, MAX_STORAGE_PATH_STR, path_template, i+j);
        }
